...
import os
import docs_configs
from tools.append_unique import append_unique # 列表不重复增加元素
from tools.is_number import is_number

def rpdesc(desc):
    for rp in docs_configs.rpdesc_text:
        desc = desc.replace(rp[0], rp[1])
    return desc

def docs_tr(file, character_type):
    ...
    for ename_config in docs_configs.ename_configs: # 替换角色英文名
...
''' 
    ...
    for removed_skin in docs_configs.removed_skins: # 移除皮肤
        if data["id"] == removed_skin[0]:
            skinlists[removed_skin[1]].remove(removed_skin[2])

    for added_skin in docs_configs.added_skins: # 增加皮肤
...
# 其他 - 文档列表
    if data["ename"] != "???": # 设置角色英文名字符串
        ...
        en_str = ""
    if int(data["id"]) < 80000 and data["id"] not in docs_configs.variation_cid:
...
# 设置类型标题
                if name_true == False: # 创建标题
                    if _quality == "限定":
                        if data["id"] in docs_configs.new_skins_cid:
                            docs_index += f'''\n### {_quality} - (未实装){skin_name}\n'''
                        else:
                            docs_index += f'''\n### {_quality} - {skin_name}\n'''
                    else:
                        docs_index += f'''\n### {_quality}\n'''
                ...
                for i in range(0,len(skinlists[_quality+_type])):
                    if i in img_true[_quality+_type]:
                        if _type == "静态立绘":
                            ...
                            docs_index += f'''- 战斗模型 `prefabs_characters_{skinlists[_quality+_type][i].lower()}`\n'''
                            used_files += [f"prefabs_characters_{skinlists[_quality+_type][i].lower()}"]

                        elif _type == "战斗效果":
                            docs_index += f'''- 战斗效果 `prefabs_effects_{skinlists[_quality+_type][i].lower()}`\n'''
                            used_files += [f"prefabs_effects_{skinlists[_quality+_type][i].lower()}"]

                        elif _type == "宿舍模型":
                            docs_index += f'''- 宿舍模型 `prefabs_dormitory_{skinlists[_quality+_type][i].lower()}`\n'''
                            used_files += [f"prefabs_dormitory_{skinlists[_quality+_type][i].lower()}"]
...
# 设置索引 - 无图像
                        ...
                        if _type == "静态立绘":
                            ...
                            if is_number(skinlists[_quality+_type][i]):
                                ...
                                for numfiles_config in docs_configs.numfiles_configs:
